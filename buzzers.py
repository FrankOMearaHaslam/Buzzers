...
def doregistration():
	...
	if request.method == "GET":
		...
		SQL = """select * from inactive_tb"""
		cursor.execute( SQL )
		if check_in_file(cursor, u):
			return render_template(
					"register.html",
					title="User {} waiting. Choose another username.".format(u))
		cursor.execute("""INSERT INTO inactive_tb VALUES(%s,%s,%s)""",(u,p,t))
		conn.commit()
...
def switch_on_users():
	...
	if request.method == "GET":
		...
		if len(userdata) == 0:
			userdata = 'No new users waiting to be enabled, well done Admin.'
		...
		for line in users_to_add:
			cursor.execute("""INSERT INTO active_tb VALUES(%s,%s,%s)""",(line[0],line[1],line[2]))
			cursor.execute("""DELETE FROM inactive_tb WHERE username = '%s' """ % line[0])
			conn.commit()
		return redirect(url_for("indexpage"))
	else:
		return render_template("index.html",
