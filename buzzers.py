...
def dologin():
    ...
    def validusertype(u2check, p2check):
        ...
        """
        with open(_VALIDUSERS) as fn:
            for line in fn:
                u, p, t = line.strip().split(',')
                if u2check == u and p2check == p:
                    return t
        # If we get this far, we didn't find the user in the file: INVALID user.
        return None
        ...
        with open(_WAITING) as fw:
            for line in fw:
                ...
                if u in request.form:
                    users_to_add.append(line)
                else:
                    users_not_to_add.append(line)
        # Remember the list of users to NOT enable.
        with open(_WAITING, "w") as fw:
            for line in users_not_to_add:
                print(line, end='', file=fw)
        # Add (append) to the list of valid users.
